...
const victoryMusicAudio = create_audio(AUDIO_BASE_URL + 'background-theme.wav', 0.35);
...
const levelClearAudio = create_audio(AUDIO_BASE_URL + 'level-clear.wav', 0.8);
...
const VICTORY_ANIMATION_DURATION = 50;
const VICTORY_ANIMATION_FPS = 30;
const VICTORY_BPM = 113.33;
const VICTORY_DELAY_FRAMES = 3 * FRAME_RATE;
...
let victoryAnimationStarted = false;
let victoryDelayPending = false;
let victoryDelayFrames = 0;
...
/**
 * 根据动画时间生成通关后的曲线画面。
 * 入参：time 为动画当前时间，范围为 0 到 VICTORY_ANIMATION_DURATION。
 * 返回值：一个可交给 curve 绘制器的彩色 Curve。
 * 边界情况：时间超出范围时仍使用正弦函数生成稳定画面。
 */
function victoryCurveAtTime(time) {
  return function (t) {
    const beat = time * VICTORY_BPM / 60;
    const pulse = 0.06 + 0.035 * (1 + math_sin(2 * math_PI * beat));
    const x = 2 * t - 1;
    const y = 0.18 * math_sin(2 * math_PI * (3 * t + beat / 4))
      + pulse * math_sin(2 * math_PI * (12 * t - beat));
    const red = 220 + 35 * math_sin(2 * math_PI * (t + beat / 8));
    const green = 90 + 90 * math_sin(2 * math_PI * (t + beat / 10));
    const blue = 35 + 80 * math_sin(2 * math_PI * (t + beat / 12));
 ...
 * 入参：无。
 * 返回值：undefined。
 * 边界情况：同一局通关时只允许启动一次，避免重复叠加声音和动画。
 */
function startVictoryAnimation() {
  ...
  stopBackgroundMusic();
  play_audio(victoryMusicAudio);
  animate_curve(
    VICTORY_ANIMATION_DURATION,
    VICTORY_ANIMATION_FPS,
...
function checkFinish() {
  for (let i = 0; i < array_length(finishObjects); i = i + 1) {
    if (finishObjects[i][RECORD_ACTIVE] && playerTouchesRecord(finishObjects[i])) {
      ...
      victoryDelayPending = true;
      victoryDelayFrames = 0;
      stopBackgroundMusic();
      play_audio(levelClearAudio);
      playClearVoice();
      if (!gameState[7]) {
...
function updateGame(currentGameState) {
  ...
  if (gameState[STATE_MODE] === MODE_WIN && victoryDelayPending) {
    victoryDelayFrames = victoryDelayFrames + 1;
    if (victoryDelayFrames >= VICTORY_DELAY_FRAMES) {
      victoryDelayPending = false;
