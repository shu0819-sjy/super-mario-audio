...
let currentLevel = 1;
let victoryBackdrop = 0;
let victoryTitle = 0;
let victorySubtitle = 0;
...
function initializeVictoryScene() {
  ...
  for (let i = 0; i < array_length(cardSettings); i = i + 1) {
    ...
    const card = update_scale(
      update_position(create_sprite(GTA6_COVER_URL), [-1000, -1000]),
      [setting[2], setting[2]]
    );
    victoryCards[i] = [card, setting[0], setting[1], setting[2], setting[3], setting[4]];
  }
  victoryTitle = update_position(create_text(''), [-1000, -1000]);
  victorySubtitle = update_position(create_text(''), [-1000, -1000]);
  return undefined;
}
 ...
 * 入参：无。
 * 返回值：undefined。
 * 边界情况：只有胜利状态显示场景，重启或死亡时全部隐藏。
 */
function updateVictoryScene() {
  if (gameState[STATE_MODE] === MODE_WIN) {
    ...
    for (let i = 0; i < array_length(victoryCards); i = i + 1) {
      ...
      const baseX = cardData[1];
      const baseY = cardData[2];
      const baseScale = cardData[3];
    ...
    for (let i = 0; i < array_length(victoryCards); i = i + 1) {
      update_position(victoryCards[i][0], [-1000, -1000]);
    }
    update_position(victoryTitle, [-1000, -1000]);
    update_position(victorySubtitle, [-1000, -1000]);
  }
