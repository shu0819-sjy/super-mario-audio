...
const GTA6_COVER_URL = 'https://raw.githubusercontent.com/shu0819-sjy/retro-reconstruction-player/main/gpt6-poster-reference.png';
...
const victoryCover = update_scale(
  update_position(create_sprite(GTA6_COVER_URL), [-1000, -1000]),
  [0.34, 0.34]
);
const victoryLabel = update_position(create_text(''), [-1000, -1000]);
 ...
 * 入参：无。
 * 返回值：undefined。
 * 边界情况：只有 MODE_WIN 显示封面，其余状态将对象移出画布。
 */
function updateVictoryCover() {
  if (gameState[STATE_MODE] === MODE_WIN) {
    const phase = get_loop_count() / FRAME_RATE;
    const tilt = 0.035 * math_sin(phase * 1.8);
    const zoom = 0.34 + 0.018 * math_sin(phase * 2.2);
    const coverX = 480 + 8 * math_sin(phase * 1.3);
    const coverY = 270 + 5 * math_cos(phase * 1.5);
    update_position(victoryCover, [coverX, coverY]);
    update_scale(victoryCover, [zoom, zoom]);
    update_rotation(victoryCover, tilt);
    update_position(victoryLabel, [480, 515]);
    update_text(victoryLabel, 'LEVEL CLEAR   MUSIC PLAYING');
  } else {
    update_position(victoryCover, [-1000, -1000]);
    update_position(victoryLabel, [-1000, -1000]);
  }
  return undefined;
}
